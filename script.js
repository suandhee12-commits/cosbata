...
function rotateCarousel() {
    cells.forEach((cell, i) => {
        ...
        const baseTransform = `rotateY(${cellAngle}deg) translateZ(${radius}px)`;
        cell.style.transform = baseTransform;
        cell.setAttribute('data-base-transform', baseTransform);
        ...
        cell.addEventListener('mouseenter', () => {
            cell.style.transform = `${baseTransform} translateZ(150px) scale(1.15)`;
        });
...
function updateTime() {
    ...
    const timeString = `${dayName} ${month} ${date}  ${hours}:${minutes} ${ampm}`;
    document.getElementById('menubar-time').textContent = timeString;
}

updateTime();
setInterval(updateTime, 1000); // 1초마다 업데이트

// 독(Dock) 아이템 클릭 이벤트 - 모달 열기
...
document.querySelectorAll('.modal').forEach(modal => {
    ...
    // 닫기 버튼 클릭시 닫기
    const closeBtn = modal.querySelector('.modal-control.close');
